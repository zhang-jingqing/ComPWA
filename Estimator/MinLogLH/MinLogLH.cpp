...
MinLogLH::MinLogLH(std::shared_ptr<Amplitude> amp_,
    std::shared_ptr<Amplitude> bkg_, std::shared_ptr<Data> data_,
    ...
    amp(amp_), ampBkg(bkg_), data(data_), phspSample(phspSample_), accSample(
        accSample_), nEvts_(0), nPhsp_(0), nStartEvt_(startEvent), nUseEvt_(
        nEvents), useFunctionTree(0), signalFraction(sigFrac), accSampleEff(0), penaltyLambda(
        0.0) {
...
std::shared_ptr<ControlParameter> MinLogLH::createInstance(
    std::shared_ptr<Amplitude> amp_, std::shared_ptr<Data> data_,
    std::shared_ptr<Data> phspSample_, unsigned int startEvent,
    unsigned int nEvents) {
  if (!instance_) {
    std::shared_ptr<Data> accSample_ = std::shared_ptr<Data>();
    instance_ = std::shared_ptr<ControlParameter>(
        new MinLogLH(amp_, std::shared_ptr<Amplitude>(), data_, phspSample_,
            accSample_, startEvent, nEvents, 1.0));
...
std::shared_ptr<ControlParameter> MinLogLH::createInstance(
  ...
  if (!instance_) {
    instance_ = std::shared_ptr<ControlParameter>(
        new MinLogLH(amp_, std::shared_ptr<Amplitude>(), data_, phspSample_,
            accSample_, startEvent, nEvents, 1.));
  }
  return instance_;
}
std::shared_ptr<ControlParameter> MinLogLH::createInstance(
    std::shared_ptr<Amplitude> amp_, std::shared_ptr<Amplitude> bkg_,
    std::shared_ptr<Data> data_, std::shared_ptr<Data> phspSample_,
    std::shared_ptr<Data> accSample_, unsigned int startEvent,
    unsigned int nEvents, double sigFrac) {
  if (!instance_) {
    instance_ = std::shared_ptr<ControlParameter>(
        new MinLogLH(amp_, bkg_, data_, phspSample_, accSample_, startEvent,
            nEvents, sigFrac));
  }
  return instance_;
}

void MinLogLH::setAmplitude(std::shared_ptr<Amplitude> amp_,
    std::shared_ptr<Data> data_, std::shared_ptr<Data> phspSample_,
    std::shared_ptr<Data> accSample_, unsigned int startEvent,
    unsigned int nEvents, bool useFuncTr, double sigFrac) {
  amp = amp_;
  data = data_;
  ...
  nPhsp_ = phspSample->getNEvents();
  if (!nUseEvt_)
    nUseEvt_ = nEvts_ - startEvent;
  if (!(startEvent + nUseEvt_ <= nEvts_))
    nUseEvt_ = nEvts_ - startEvent;
  if (!(startEvent + nUseEvt_ <= nPhsp_))
    nUseEvt_ = nPhsp_ - startEvent;
  mData = data->getMasses();
  mPhspSample = phspSample->getMasses();
  if (accSample) {
    mAccSample = accSample->getMasses();
  ...
  if (data->hasWeights() && signalFraction != 1.)
    throw std::runtime_error(
        "MinLogLH::MinLogLhbkg() data sample has weights and signal fraction !=1. That makes no sense!");
  calcSumOfWeights();

  if (signalFraction != 1 && !ampBkg)
    throw std::runtime_error(
        "MinLogLH::setAmplitude() a signal fraction smaller 1 was set"
            " but no background description given. If you want to assume a flat background, "
            "pass a Physics//Background//FlatBackground object!");
  if (signalFraction == 1 && !ampBkg)
    ampBkg = std::shared_ptr<Amplitude>();
...
void MinLogLH::setAmplitude(std::shared_ptr<Amplitude> amp_,
    ...
    double sigFrac) {
  ampBkg = bkg_;
  return setAmplitude(amp_, data_, phspSample_, accSample_, startEvent, nEvents,
      useFuncTr, sigFrac);
}
...
void MinLogLH::iniLHtree() {
  ...
  //----Strategies needed
  std::shared_ptr<MultAll> mmultStrat = std::shared_ptr<MultAll>(
      new MultAll(ParType::MCOMPLEX));
  std::shared_ptr<MultAll> mmultDStrat = std::shared_ptr<MultAll>(
      new MultAll(ParType::MDOUBLE));
  std::shared_ptr<AddAll> multiDoubleAddStrat = std::shared_ptr<AddAll>(
      new AddAll(ParType::MDOUBLE));
  std::shared_ptr<AddAll> multiComplexAddStrat = std::shared_ptr<AddAll>(
      new AddAll(ParType::MCOMPLEX));
  std::shared_ptr<AbsSquare> msqStrat = std::shared_ptr<AbsSquare>(
      new AbsSquare(ParType::MDOUBLE));
  std::shared_ptr<LogOf> mlogStrat = std::shared_ptr<LogOf>(
      new LogOf(ParType::MDOUBLE));
  std::shared_ptr<MultAll> multStrat = std::shared_ptr<MultAll>(
      new MultAll(ParType::COMPLEX));
  std::shared_ptr<MultAll> multDStrat = std::shared_ptr<MultAll>(
      new MultAll(ParType::DOUBLE));
  std::shared_ptr<AddAll> addStrat = std::shared_ptr<AddAll>(
      new AddAll(ParType::DOUBLE));
  std::shared_ptr<AddAll> addComplexStrat = std::shared_ptr<AddAll>(
      new AddAll(ParType::COMPLEX));
  std::shared_ptr<AbsSquare> sqStrat = std::shared_ptr<AbsSquare>(
      new AbsSquare(ParType::DOUBLE));
  std::shared_ptr<LogOf> logStrat = std::shared_ptr<LogOf>(
      new LogOf(ParType::DOUBLE));
  std::shared_ptr<Complexify> complStrat = std::shared_ptr<Complexify>(
      new Complexify(ParType::COMPLEX));
  std::shared_ptr<Inverse> invStrat = std::shared_ptr<Inverse>(
      new Inverse(ParType::DOUBLE));
  std::shared_ptr<SquareRoot> sqRootStrat = std::shared_ptr<SquareRoot>(
      new SquareRoot(ParType::DOUBLE));
  ...
  if (!accSample) {    //binned
    ...
    signalPhspTree->createLeaf("InvNmc", 1 / ((double) mPhspSample.nEvents),
        "normFactor");
    ...
    signalPhspTree->createNode("IntensPhsp", msqStrat, "sumAmp",
        mAccSample.nEvents, false);    //|T_{ev}|^2
    signalPhspTree->createLeaf("InvNmc",
        1 / ((double) mAccSample.nEvents / accSampleEff), "normFactor");
  ...
  if (ampBkg) {
    ...
    bkgPhspTree->createLeaf("phspVolume",
        Kinematics::instance()->getPhspVolume(), "normFactor");
    if (!accSample) {    //binned
      bkgPhspTree_amp = ampBkg->getAmpTree(mPhspSample, mPhspSample, "_Phsp");
      eff = std::shared_ptr<MultiDouble>(
          new MultiDouble("eff", mPhspSample.eff));
      bkgPhspTree->createLeaf("InvNmc", 1 / ((double) mPhspSample.nEvents),
          "normFactor");
      ...
      bkgPhspTree->createNode("IntensPhsp", msqStrat, "sumAmp",
          mAccSample.nEvents, false);    //|T_{ev}|^2
  ...
  //------------Setup Tree Pars---------------------
  std::shared_ptr<MultiDouble> weight = std::shared_ptr<MultiDouble>(
      new MultiDouble("weight", mData.weight));
  ...
  physicsTree->createNode("weightLog", mmultDStrat, "sumEvents", mData.nEvents,
  ...
  physicsTree->createNode("addBkgSignal", multiDoubleAddStrat, "Log",
      mData.nEvents, false);    //I_{ev} = x_{ev} + (1-f_{bkg})

  //signal term
  physicsTree->createNode("normIntens", mmultDStrat, "addBkgSignal",
  ...
  if (ampBkg) {
    ...
    physicsTree->createNode("IntensBkg", msqStrat, "normBkg", mData.nEvents,
        false);
...
//	BOOST_LOG_TRIVIAL(debug) << std::endl << treeString;
  if (!physicsTree->sanityCheck()) {
    throw std::runtime_error(
        "MinLogLH::iniLHtree() tree has structural problems. Sanity check not passed!");
  }
...
double MinLogLH::controlParameter(ParameterList& minPar) {
  ...
  if (!useFunctionTree) {
    ...
    for (unsigned int evt = nStartEvt_; evt < nUseEvt_ + nStartEvt_; evt++) {    //loop over data sample
      ...
      if (intens > 0)
        sumLog += std::log(
            signalFraction * intens / norm
                + (1 - signalFraction) * intensBkg / normBkg)
            * theEvent.getWeight();
    ...
    std::shared_ptr<DoubleParameter> logLH = std::dynamic_pointer_cast<
        DoubleParameter>(physicsTree->head()->getValue());
    lh = logLH->GetValue();
  }
  lh += calcPenalty();
