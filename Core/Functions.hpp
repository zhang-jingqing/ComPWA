    ...
    case ParType::DOUBLE: {
      ...
      out = std::shared_ptr<AbsParameter>(
          new DoubleParameter(out->GetName(),
              std::log(paras.GetParameterValue(0))));
      break;
    }        //end double
...
public:
  ...
  virtual bool execute(ParameterList& paras,
    ...
    case ParType::MCOMPLEX: {
      ...
      unsigned int nElements = paras.GetMultiDouble(0)->GetNValues();
      //fill MultiDouble parameter
      std::vector<std::complex<double> > results(nElements,
  ...
  virtual bool execute(ParameterList& paras,
    ...
    if (nMC + nMD + nD + nI + nC == 0)
      throw std::runtime_error(
          "AbsSquare Strategy: Input parameter list is empty!");

    //only one parameter possible
    if ((nMC + nMD + nD + nI + nC) > 1)
      throw std::runtime_error(
          "AbsSquare Strategy: More then one type of parameter in input!");
    ...
    case ParType::MDOUBLE: {
      ...
      if (!nMD && !nMC)
        throw std::runtime_error(
            "AbsSquare Strategy: Requested output is MDOUBLE but no MDOUBLE or MCOMPLEX was given as input!");
      if (nMD) {
        unsigned int nElements = paras.GetMultiDouble(0)->GetNValues();
        //fill MultiDouble parameter
        std::vector<double> results(nElements, 0.);
    ...
    case ParType::DOUBLE: {
      ...
      if (!nD && !nC)
        throw std::runtime_error(
            "AbsSquare Strategy: Requested output is DOUBLE but no DOUBLE or COMPLEX was given as input!");
      if (nD) {
